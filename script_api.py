    ...
    for name, transformer, columns in ct.transformers_:
        if name == 'remainder':
            ...
            if transformer == 'passthrough':
                remainder_features = [original_features[i] if isinstance(i, int) else i for i in columns]
                feature_names.extend(remainder_features)
            continue
        if hasattr(transformer, 'get_feature_names_out'):
            input_features = [original_features[i] if isinstance(i, int) else i for i in columns]
            ...
            feature_names.extend([original_features[i] if isinstance(i, int) else i for i in columns])
...
def predict(df, seuil):
    ...
    shap_df = group_shap_by_original_features(shap_values, feature_mapping, df.columns).set_index(df.index)

    # Joindre les prédictions aux valeurs shap par variable
    resultat_df = y_pred_df.merge(shap_df, left_index=True, right_index=True, how='left')
